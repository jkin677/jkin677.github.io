...
Blockly.InsertionMarkerManager.prototype.createMarkerBlock_ = function(sourceBlock) {
  ...
  try {
	...
	// Synchronise inputs for multiprocessing block
	if (sourceBlock.type == 'multiprocess' || sourceBlock.type == 'run_function_pointer'){
		if (sourceBlock.returns == false){
			result.setOutput(false)
			result.setNextStatement(true, null);
			result.setPreviousStatement(true, null);
		}
		if (sourceBlock.type == 'multiprocess' && sourceBlock.inputs.length != 0){
			result.removeInput(result.numIterations.name)
		}
		for (var i = 0; i < sourceBlock.inputs.length; i++){
			result.appendValueInput("DATA" + i.toString())
					.setCheck("Array")
					.appendField('Values for Argument '+ i.toString())
		}
	}
