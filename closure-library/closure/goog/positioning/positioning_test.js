...
function testPositionAtAnchorWithOffset() {
  ...
  assertRoundedEquals(
      'Top edge of popup should be twenty pixels from anchor.', anchorRect.top,
      popupRect.top + 20);
...
function testPositionAtAnchorWithMargin() {
  ...
  var popupRect = goog.style.getBounds(popup);
  assertRoundedEquals(
      'Left edge of popup should be four pixels from anchor.', anchorRect.left,
      popupRect.left - 4);
  assertRoundedEquals(
...
function testAdjustForViewportFailIgnore() {
  ...
  assertEquals(
      'Viewport overflow should be ignored.',
      goog.positioning.OverflowStatus.NONE, f(pos, size, viewport, overflow));
...
function testAdjustForViewportResizeHeight() {
  ...
  size = newSize(50, 50);
  assertEquals(
      'No Viewport overflow.', goog.positioning.OverflowStatus.NONE,
      f(pos, size, viewport, overflow));
  assertTrue(
      'Output box is within viewport',
      viewport.contains(
...
function testAdjustForViewportResizeWidth() {
  ...
  assertEquals(
      'Viewport width should be resized.',
...
function testPositionAtAnchorWithResizeHeight() {
  ...
  var status = goog.positioning.positionAtAnchor(
      ...
      goog.positioning.Overflow.RESIZE_HEIGHT, null, viewport.toBox());
  assertEquals(
      'Status should be HEIGHT_ADJUSTED.',
  ...
  assertTrue(
      'Popup ' + goog.style.getBounds(popup) + ' not is within viewport' +
          viewport,
...
function testPositionAtAnchorFrameViewportStandard() {
  ...
  var popupRect = goog.style.getBounds(popup);
  assertEquals(
      'Status should not have any ADJUSTED and FAILED.',
...
function testPositionAtAnchorNestedFrames() {
  ...
  var popupRect = goog.style.getBounds(popup);
  assertRoundedEquals(
      'Top of frame should align with bottom of the popup', innerIframeRect.top,
...
function testPositionAtAnchorOffscreen() {
  ...
  assertObjectEquals(newCoord(offset, offset), goog.style.getPageOffset(popup));

  goog.positioning.positionAtAnchor(
      anchor, corner.TOP_LEFT, popup, corner.TOP_LEFT, null, null,
      overflow.ADJUST_X_EXCEPT_OFFSCREEN | overflow.ADJUST_Y);
  assertObjectEquals(newCoord(-1000, offset), goog.style.getPageOffset(popup));

  goog.positioning.positionAtAnchor(
      anchor, corner.TOP_LEFT, popup, corner.TOP_LEFT, null, null,
      overflow.ADJUST_X | overflow.ADJUST_Y_EXCEPT_OFFSCREEN);
  assertObjectEquals(newCoord(offset, -1000), goog.style.getPageOffset(popup));

  goog.positioning.positionAtAnchor(
      anchor, corner.TOP_LEFT, popup, corner.TOP_LEFT, null, null,
      overflow.ADJUST_X_EXCEPT_OFFSCREEN | overflow.ADJUST_Y_EXCEPT_OFFSCREEN);
  assertObjectEquals(newCoord(-1000, -1000), goog.style.getPageOffset(popup));
}
...
function testPositionAtAnchorWithOverflowScrollOffsetParent() {
  ...
  assertObjectRoughlyEquals(
      ...
      goog.style.getPageOffset(popup), 1);
  ...
  goog.positioning.positionAtAnchor(
      ...
      overflow.ADJUST_X | overflow.ADJUST_Y);
  assertObjectRoughlyEquals(
      new goog.math.Coordinate(testAreaOffset.x + 400, testAreaOffset.y + 400),
      goog.style.getPageOffset(popup), 1);
