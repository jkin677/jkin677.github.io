...
Blockly.Procedures.getAllProcedureModels = function (root){
	var nonReturnProcedures = root.getBlocksByType('procedures_defnoreturn') || []
	var returnProcedures = root.getBlocksByType('procedures_defreturn')  || []
	
	var procedures = nonReturnProcedures.concat(returnProcedures)
	
	for (var i=0; i< procedures.length; i++){
		procedures[i] = new Blockly.FunctionModel(root, procedures[i].getFieldValue('NAME'), '', procedures[i].id)
	}
 ...
 * @param {!String} name to search by
 * @return {!Blockly.Block} The block with corresponding ID, or null of not found
 */
Blockly.Procedures.getProcedureByName = function(workspace, name){
	var ProcedureBlocks = workspace.getBlocksByType('procedures_defreturn') || [];
	ProcedureBlocks.concat(workspace.getBlocksByType('procedures_defnoreturn') || [])
	
	// Search through procedures that return
	for (var i = 0; i < ProcedureBlocks.length; i++){
		if (ProcedureBlocks[i].getFieldValue('NAME') == name){
			return ProcedureBlocks[i]
		}
	}
